...
-- ----------------------------
-- Function structure for `fn_get_child`
-- ----------------------------
DROP FUNCTION IF EXISTS `fn_get_child`;
DELIMITER ;;
CREATE DEFINER=`groundpush`@`%` FUNCTION `fn_get_child`(rootId INT) RETURNS varchar(4000) CHARSET utf8
BEGIN
    #声明两个临时变量
...
-- ----------------------------
-- Function structure for `fn_recursive_query_customer`
-- ----------------------------
DROP FUNCTION IF EXISTS `fn_recursive_query_customer`;
DELIMITER ;;
CREATE DEFINER=`groundpush`@`%` FUNCTION `fn_recursive_query_customer`(rootId INT) RETURNS varchar(4000) CHARSET utf8
BEGIN
    #声明两个临时变量
    DECLARE temp VARCHAR(4000);
    DECLARE temp_child VARCHAR(4000);
