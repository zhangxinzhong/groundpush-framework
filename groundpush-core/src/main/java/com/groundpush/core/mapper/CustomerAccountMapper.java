...
public interface CustomerAccountMapper {
     ...
     * @return
     */
    @Select(" select ca.*,ifnull((select sum(cl.amount)*100 from t_cashout_log cl where cl.customer_id=#{customerId}),0)+ifnull(ca.amount,0) as total_amount from t_customer_account ca where ca.customer_id=#{customerId} ")
