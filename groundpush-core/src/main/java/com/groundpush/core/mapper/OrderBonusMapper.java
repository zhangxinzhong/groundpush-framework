...
public interface OrderBonusMapper {
    ...
    /**
     * 通过订单id 获取所有订单关联的分成信息
     * @param orderId
     * @return
     */
    @Select(" select a.bonus_id,a.order_id,(select c.login_no from t_customer_login_account c where c.type=1 and c.customer_id = a.customer_id) customer_login_no,a.bonus_amount,a.bonus_type,a.status,a.created_time,(select c.login_no from t_customer_login_account c where c.type=1 and c.customer_id = a.bonus_customer_id) bonus_customer_login_no from t_order_bonus a left join t_customer b on a.customer_id = b.customer_id where a.order_id = #{orderId} ")
